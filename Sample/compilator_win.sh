...
while true; do
    ...
    : ' search for pawncc.exe binary within the current script directory '
    while ifs= read -r -d '' pawncc; do
        if [ -f "$pawncc" ]; then
            bashpawncc="$pawncc"  : ' found pawncc.exe, store the path '
            break
        fi
    done < <(find "$shell_dir" -name "pawncc.exe" -print0)
